...
export (float, 1000) var pickup_range := 0.5 setget set_pickup_range
export (float, 4) var impulse_factor := 1.0
export (Buttons) var pickup_button_id = Buttons.VR_GRIP
export (Buttons) var action_button_id = Buttons.VR_TRIGGER
export (int, 10) var max_samples := 5
	...
	if !picked_up_object:
		...
		for o in object_in_area:
			...
			if o.is_picked_up() == false:
				var distance : float = (o.global_transform.origin - global_transform.origin).length()
...
func _ready():
	...
	var err := parent.connect("button_pressed", self, "_on_button_pressed")
	assert(not err)
	err = parent.connect("button_release", self, "_on_button_release")
	assert(not err)
